...
class KerasLinear(KerasPilot):
    ...
    def __init__(self, num_outputs=2, input_shape=(224, 224, 3), roi_crop=(0, 0), *args, **kwargs):
        ...
        self.model = self.default_n_linear(num_outputs, input_shape, roi_crop)
        self.compile()
    ...
    def run(self, img_arr):
        ...
        # reduction = np.squeeze(img_arr, axis=0)
        # img_arr = cv2.resize(reduction, (160, 120))
        # img_arr = np.expand_dims(img_arr, axis=0)
        outputs = self.model.predict(img_arr) #img_arr/255.0
        steering = outputs[0]
        throttle = outputs[1]*1.0
        print("steerin {} throttle {}".format(steering, throttle))
        return steering[0][0], throttle[0][0]


    def default_n_linear(self, num_outputs, input_shape=(120, 160, 3), roi_crop=(0, 0)):
